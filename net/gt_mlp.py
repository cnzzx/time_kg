...
class GT_MLPNet(nn.Module):
    def __init__(self, net_params):
        ...
        self.embedding_h = nn.Linear(768, hidden_dim)
        ...
        self.MLP_layer = MLPReadout(15, 1)   # 1 out dim since regression problem        
        
    def forward(self, device, g, h, e, x, start_bucket, end_bucket, k=0.25, time_gap=7, h_lap_pos_enc=None, h_wl_pos_enc=None):
        """
        g: the graph structure
        g.ndata: the vertex feature dict
                 There should be the:
                 - start date of one event (g.ndata['date'])
        ...
        event_influence = torch.zeros((end_bucket - start_bucket + 1), requires_grad=True).to(device)
        event_dates = g.ndata['date']

        n_events = event_dates.size()[0]
        influence_span = int(1 / k)
        for event_idx in range(n_events):
            event_bucket = get_time_bucket(event_dates[event_idx].item(), time_gap)
            ...
            for inf_dis in range(influence_span):
                ...
                # linear degradation
        enhanced_series = torch.cat((x.unsqueeze(1), event_influence[:input_seq_len].unsqueeze(1)), axis=1)
        enhanced_series = self.fn_embedding_xh(enhanced_series)
        final_feat = torch.cat((enhanced_series, event_influence[input_seq_len:].unsqueeze(1)), axis=0).squeeze().unsqueeze(0)
